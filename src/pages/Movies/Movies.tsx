import { Search, Play, Star, Calendar, Clock, Heart } from 'lucide-react'; // YANGI: Heart qo'shildi
import { PATH } from '../../components';
import type { Root2 } from '../../@types';
import { useQuery } from '@tanstack/react-query';
import { useContext, useState, useMemo, type ChangeEvent } from 'react'; // useState, useMemo qo'shildi
...
export default function Movies() {
    ...
    // TUZATILDI: search va yil filter uchun alohida state'lar
    const [searchValue, setSearchValue] = useState<string>('')
    const [selectedYear, setSelectedYear] = useState<string>('')

    // TUZATILDI: useMemo — movies, search yoki yil o'zgarganda qayta hisoblaydi
    const filteredMovies = useMemo(() => {
        ...
        return movies.filter((item: Root2) => {
            ...
            // TUZATILDI: String(selectedYear) — year context'da number bo'lgani uchun
            const matchesYear = selectedYear === '' || selectedYear === 'Hammasi'
                ? true
                : item.show.premiered?.startsWith(String(selectedYear))

            // Ikkalasi ham mos kelsa ko'rsatiladi
            return matchesSearch && matchesYear
        })
    }, [movies, searchValue, selectedYear])

    // TUZATILDI: search input handler
    function handleSearch(e: ChangeEvent<HTMLInputElement>) {
        setSearchValue(e.target.value)
    }

    // TUZATILDI: yil select handler
        ...
        <div className="w-full min-h-screen bg-[#0a0a0c] text-white pt-5 px-4 pb-16">
            ...
            <div className="max-w-7xl mx-auto mb-12 flex gap-4 justify-between items-center bg-white/5 p-4 rounded-2xl border border-white/10 backdrop-blur-md">
                <div className="relative w-full md:w-96">
                    {/* TUZATILDI: value va onChange ulandi */}
                    <input
                        value={searchValue}
                ...
                <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    {/* TUZATILDI: value va onChange ulandi, value={String(item.year)} number→string */}
                    <select
                        value={selectedYear}
                    ...
                    >
                        ...
                        {year.map(item => (
                            // TUZATILDI: key va value={String(item.year)} qo'shildi
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {/* TUZATILDI: movies o'rniga filteredMovies ishlatildi */}
